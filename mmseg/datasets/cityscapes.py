...
@DATASETS.register_module()
class CityscapesIndoDataset(CustomDataset):
    """Cityscapes Indonesia dataset.

    The ``img_suffix`` is fixed to '_leftImg8bit.png' and ``seg_map_suffix`` is
    fixed to '_gtFine_labelTrainIds.png' for Cityscapes Indonesia dataset.
    """
    ...
    def __init__(self,
        ...
        if crop_pseudo_margins is not None:
            assert kwargs['pipeline'][-1]['type'] == 'Collect'
            kwargs['pipeline'][-1]['keys'].append('valid_pseudo_mask')
        super(CityscapesIndoDataset, self).__init__(
            img_suffix=img_suffix, seg_map_suffix=seg_map_suffix, **kwargs)
    ...
    def pre_pipeline(self, results):
        super(CityscapesIndoDataset, self).pre_pipeline(results)